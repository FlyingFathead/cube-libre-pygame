...
# Later levels add one module per level, but each new module must be a real
# 90-degree X/Z-plane L-turn from the previous module. No repeated same-direction
# straight pipe pieces are allowed in this generator.
LEVEL_DIRECTIONS = ((1, 0),)  # legacy/debug fallback; procedural route generation is used below.
COURSE_ROUTE_SEED_BASE = 0xC0BE51DE
COURSE_ROUTE_SEED_STEP = 7919
...
def _turn_left_dir(direction):
    dx, dz = _route_dir_key(direction)
    return (-dz, dx)


def _turn_right_dir(direction):
    dx, dz = _route_dir_key(direction)
    return (dz, -dx)
...
def _route_candidate_end(start_x: float, start_z: float, direction):
    dx, dz = _route_dir_key(direction)
    length = _route_module_length()
    return (start_x + dx * length, start_z + dz * length)


def _route_occupied_rect_for_candidate(start_x: float, start_z: float, direction):
    """Approximate a candidate module footprint in the world X/Z plane.

    This is only for procedural route planning. The actual playable volume is
    still handled by CourseModule, module_pipe_lx_span(), point_inside_course(),
    and the explicit turn-joint functions.
    """
    dx, dz = _route_dir_key(direction)
    end_x, end_z = _route_candidate_end(start_x, start_z, direction)
    ...
    if dx != 0:
        ...
        xmin = start_x - side
        xmax = start_x + side
        zmin = min(start_z, end_z)
        zmax = max(start_z, end_z)

    # Include the far joint area because L-corners are same-cross-section cubes.
    xmin = min(xmin, end_x - joint)
    xmax = max(xmax, end_x + joint)
    zmin = min(zmin, end_z - joint)
    zmax = max(zmax, end_z + joint)

    return (xmin, xmax, zmin, zmax)


def _route_rects_overlap(a, b) -> bool:
    ax0, ax1, az0, az1 = a
    bx0, bx1, bz0, bz1 = b
    return not (
        ax1 <= bx0 or
...
def _route_candidate_clear(candidate_rect, existing_rects) -> bool:
    """Reject overlap with older modules.

    Ignore the immediately previous module because the new module must share the
    L-joint at the previous module's end.
    """
    older_rects = existing_rects[:-1]
    for rect in older_rects:
        if _route_rects_overlap(candidate_rect, rect):
            return False
    return True


def _route_bounds(rects):
    return (
...
def _route_expands_bounds(candidate_rect, old_bounds, direction) -> bool:
    """Return True when candidate grows the current occupied X/Z footprint."""
    if old_bounds is None:
        return True

    xmin, xmax, zmin, zmax = candidate_rect
    old_xmin, old_xmax, old_zmin, old_zmax = old_bounds
    dx, dz = _route_dir_key(direction)

    margin = 0.25

    if dx > 0:
        return xmax > old_xmax + margin
    if dx < 0:
        return xmin < old_xmin - margin
    if dz > 0:
        return zmax > old_zmax + margin
    if dz < 0:
        return zmin < old_zmin - margin

    return False


def make_course_directions_for_level(level: int):
    """Return deterministic X/Z-plane module directions for this level.

    Level 1 preserves the original straight +X tunnel. Later modules are
    generated as 90-degree left/right turns only, so the route cannot become
    two consecutive same-direction straight pipe pieces.
    """
    level = max(1, int(level))
    if level <= 1:
        return ((1, 0),)

    rng = random.Random(COURSE_ROUTE_SEED_BASE + level * COURSE_ROUTE_SEED_STEP)

    directions = [(1, 0)]

    # Simulate placement so the route planner can reject obvious fold-backs.
    sx, sz = COURSE_X_MIN, 0.0
    first_rect = _route_occupied_rect_for_candidate(sx, sz, directions[0])
    existing_rects = [first_rect]
    sx, sz = _route_candidate_end(sx, sz, directions[0])

    current_dir = directions[0]

    for _idx in range(1, level):
        # Critical rule: only left/right 90-degree turns. No current_dir here.
        candidates = [
            _turn_left_dir(current_dir),
            _turn_right_dir(current_dir),
        ]
        rng.shuffle(candidates)

        old_bounds = _route_bounds(existing_rects)
        scored = []

        # Pass 1: clear and outward-growing turns only.
        for cand in candidates:
            rect = _route_occupied_rect_for_candidate(sx, sz, cand)
            if not _route_candidate_clear(rect, existing_rects):
                continue
            if not _route_expands_bounds(rect, old_bounds, cand):
                continue

            xmin, xmax, zmin, zmax = rect
            bx0, bx1, bz0, bz1 = old_bounds
            growth = (
                max(0.0, bx0 - xmin) +
                max(0.0, xmax - bx1) +
                max(0.0, bz0 - zmin) +
            ...
            scored.append((1000.0 + growth + rng.random(), cand, rect))

        # Pass 2: if no outward turn exists, allow any clear 90-degree turn.
        # Still no straight candidate.
        if not scored:
            for cand in candidates:
                rect = _route_occupied_rect_for_candidate(sx, sz, cand)
                if not _route_candidate_clear(rect, existing_rects):
                    continue
                scored.append((100.0 + rng.random(), cand, rect))

        # Pass 3: preserve the design rule even when the footprint planner paints
        # itself into a corner: choose a turn anyway, never a straight duplicate.
        if not scored:
            cand = candidates[0]
            rect = _route_occupied_rect_for_candidate(sx, sz, cand)
            scored.append((0.0, cand, rect))

        scored.sort(reverse=True, key=lambda item: item[0])
        _score, chosen, chosen_rect = scored[0]

        directions.append(chosen)
        existing_rects.append(chosen_rect)
        sx, sz = _route_candidate_end(sx, sz, chosen)
        current_dir = chosen

    if COURSE_ROUTE_DEBUG:
        print(f"[DEBUG] level {level} route: {directions}")
